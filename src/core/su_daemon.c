...
#define DEFAULT_FOLD6_KSUD_PATH "/data/local/tmp/ksud-zfold6-F9560ZCS4DZG3-samsung-main-no-patch-text-kdp"
#define APP_FOLD6_KSUD_PATH "/data/user/0/com.ghostlock.app/files/ksud-zfold6-F9560ZCS4DZG3-samsung-main-no-patch-text-kdp"
#define LOGCAT_PATH "/system/bin/logcat"
#define TRACE_PATH "/data/local/tmp/.ghostlock_ksu.log"
#define STAGE_TMP_PATH "/data/local/tmp/.ksud-stage.tmp"
#define STAGE_PATH "/data/local/tmp/.ksud-stage"
#define KSU_KMI "android14-6.1"
...
/*
 * The adb transport disappears when the phone reboots.  Keep the daemon's
 * phase markers in a root-owned file so the first failing phase survives the
 * reboot and is not confused with an older terminal log.
 */
static void trace_log(const char *format, ...) {
  int fd = open(TRACE_PATH, O_WRONLY | O_CREAT | O_APPEND | O_CLOEXEC, 0644);
  if (fd < 0) {
    return;
  }
  va_list ap;
...
static void trace_reset(void) {
  int fd = open(TRACE_PATH, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
  if (fd < 0) {
    return;
  }
  dprintf(fd, "[*] trace reset pid=%d\n", getpid());
  fsync(fd);
  close(fd);
}
...
/* Match the original Root-My-Galaxy flow: refresh the hidden stage file
 * immediately before late-load so ksud never consumes a stale payload left
 * by an earlier run. */
static const char *request_env_value(const struct su_request *request,
                                     const char *name) {
  if (!request || !request->envp || !name) return NULL;
  size_t name_len = strlen(name);
  for (size_t i = 0; request->envp[i]; i++) {
    const char *entry = request->envp[i];
...
static const char *select_ksud_source(const struct su_request *request) {
  const char *from_app = request_env_value(request, "GHOSTLOCK_KSUD_PATH");
  if (from_app && from_app[0] && access(from_app, X_OK) == 0) {
    trace_log("[*] ksud source selected from app env path=%s\n", from_app);
    return from_app;
  }

  static const char *const candidates[] = {
      DEFAULT_FOLD6_KSUD_PATH,
      APP_FOLD6_KSUD_PATH,
  ...
  for (size_t i = 0; candidates[i]; i++) {
    ...
    if (access(candidates[i], X_OK) == 0) {
      trace_log("[*] ksud source selected path=%s\n", candidates[i]);
      return candidates[i];
    }
    trace_log("[*] ksud source unavailable path=%s errno=%d\n",
              candidates[i], errno);
  }
  trace_log("[!] ksud source not found\n");
  if (from_app && from_app[0]) return from_app;
  return NULL;
}

static int stage_loader(const char *source_path) {
  if (!source_path) return 0;
  int src = open(source_path, O_RDONLY | O_CLOEXEC);
  if (src < 0) {
    trace_log("[!] stage source open failed path=%s errno=%d\n",
              source_path, errno);
    return 0;
  }

  int dst = open(STAGE_TMP_PATH,
                 O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0755);
  if (dst < 0) {
    trace_log("[!] stage temp open failed path=%s errno=%d\n",
              STAGE_TMP_PATH, errno);
    close(src);
    return 0;
  }
  if (fchmod(dst, 0755) != 0) {
    trace_log("[!] stage temp chmod failed errno=%d\n", errno);
    close(src);
    close(dst);
  ...
  for (;;) {
    ...
    if (n < 0 || !write_full(dst, buffer, (size_t)n)) {
      trace_log("[!] stage copy failed errno=%d bytes=%llu\n", errno,
  ...
  if (fsync(dst) != 0) {
    trace_log("[!] stage fsync failed errno=%d bytes=%llu\n", errno,
              (unsigned long long)total);
    ok = 0;
  }
  close(src);
  ...
  if (rename(STAGE_TMP_PATH, STAGE_PATH) != 0) {
    trace_log("[!] stage rename failed src=%s dst=%s errno=%d\n",
              STAGE_TMP_PATH, STAGE_PATH, errno);
    unlink(STAGE_TMP_PATH);
    return 0;
  }
...
/*
 * Root-My-Galaxy's Fold6 late-load boundary:
 *
 * The UMH process is already uid 0.  It creates a private mount namespace,
 * bind-mounts the target ksud over logcat, and lets ksud perform its normal
 * late-load sequence.  Keeping the bind mount private is important: the
 * shell-side client must not replace the system logcat mount globally.
 */
static int run_fold6_late_load(struct su_request *request, int conn) {
  const char *ksud_path = select_ksud_source(request);
  trace_log("[*] original late-load begin conn=%d uid=%d euid=%d ksud=%s\n",
            conn, getuid(), geteuid(), ksud_path ? ksud_path : "<none>");
  if (getuid() != 0 || geteuid() != 0) {
    trace_log("[!] original late-load worker is not root\n");
    return 126;
  }

  struct stat ksud_stat;
  if (!ksud_path || stat(ksud_path, &ksud_stat) != 0) {
    trace_log("[!] original ksud stat failed path=%s errno=%d\n",
              ksud_path ? ksud_path : "<none>", errno);
    dprintf(STDERR_FILENO, "late-load: ksud unavailable: %s\n",
            strerror(errno));
    return 1;
  }
  trace_log("[*] original ksud ready path=%s bytes=%llu mode=%o\n",
            ksud_path, (unsigned long long)ksud_stat.st_size,
            (unsigned)(ksud_stat.st_mode & 07777));
  if (!stage_loader(ksud_path)) {
    trace_log("[!] original ksud stage refresh failed\n");
    return 9;
  }

  pid_t pid = fork();
  if (pid < 0) {
    trace_log("[!] original late-load fork failed errno=%d\n", errno);
    return 1;
  }
  if (pid == 0) {
    if (dup2(request->stdin_fd, STDIN_FILENO) < 0 ||
      ...
      trace_log("[!] original late-load child setup failed errno=%d\n", errno);
      _exit(126);
    }
    close(conn);
    close_request_fds(request);

    trace_log("[*] original late-load private mount begin pid=%d\n", getpid());
    if (unshare(CLONE_NEWNS) != 0 ||
        mount(NULL, "/", NULL, MS_REC | MS_PRIVATE, NULL) != 0) {
      trace_log("[!] original late-load private mount failed errno=%d\n",
                errno);
    ...
    if (mount(ksud_path, LOGCAT_PATH, NULL, MS_BIND, NULL) != 0) {
      trace_log("[!] original late-load bind failed src=%s dst=%s errno=%d\n",
                ksud_path, LOGCAT_PATH, errno);
      dprintf(STDERR_FILENO, "late-load: bind mount: %s\n", strerror(errno));
      _exit(11);
    }
    trace_log("[*] original late-load bind mounted src=%s dst=%s\n",
              ksud_path, LOGCAT_PATH);

    pid_t loader = fork();
    if (loader < 0) {
      trace_log("[!] original late-load ksud fork failed errno=%d\n", errno);
    ...
    trace_log("[*] original late-load ksud exit=%d\n", loader_status);
    if (loader_status != 0) {
      _exit(loader_status);
    }
  ...
  trace_log("[*] original late-load worker complete status=%d\n", status);
  return status;
}
...
static void serve_one(int conn) {
  ...
  if (!get_peer_cred(conn, &peer) || peer.uid != allowed_client_uid) {
    trace_log("[!] serve peer rejected uid=%d expected=%d errno=%d\n",
              peer.uid, allowed_client_uid, errno);
    char denied = 'D';
    write_full(conn, &denied, sizeof(denied));
    return;
  }
  trace_log("[*] serve peer accepted uid=%d pid=%d\n", peer.uid, peer.pid);
  char allowed = 'A';
  if (!write_full(conn, &allowed, sizeof(allowed))) {
    trace_log("[!] serve auth response failed errno=%d\n", errno);
    return;
  }
  ...
  if (recv(conn, &operation, sizeof(operation), MSG_PEEK) ==
      ...
      operation == 'H') {
    trace_log("[*] serve hold request\n");
    if (!read_full(conn, &operation, sizeof(operation))) {
      trace_log("[!] serve hold marker read failed errno=%d\n", errno);
      return;
    }
    hold_kernel_references(conn);
    trace_log("[*] serve hold request returned\n");
    return;
  }

  struct su_request request;
  if (!recv_request(conn, &request)) {
    trace_log("[!] serve request decode failed errno=%d\n", errno);
  ...
  int status = is_kernelsu_late_load
                   ? run_fold6_late_load(&request, conn)
...
static int daemon_main(void) {
  ...
  int fd = socket(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0);
  if (fd < 0) {
    trace_log("[!] daemon socket failed errno=%d\n", errno);
    return 1;
  }
  ...
  if (bind(fd, (struct sockaddr *)&sun, sizeof(sun)) != 0 ||
      listen(fd, 16) != 0) {
    trace_log("[!] daemon bind/listen failed errno=%d\n", errno);
    close(fd);
    return 1;
  }
  chmod(BOOTSTRAP_SOCK_PATH, 0666);
  trace_log("[*] daemon socket ready path=%s\n", BOOTSTRAP_SOCK_PATH);
  ...
  for (;;) {
    ...
    if (conn < 0) {
      trace_log("[!] daemon accept failed errno=%d\n", errno);
      sleep(1);
      continue;
    }
    trace_log("[*] daemon accepted conn=%d\n", conn);

    pid_t pid = fork();
    if (pid == 0) {
      trace_log("[*] daemon connection worker pid=%d\n", getpid());
      close(fd);
      serve_one(conn);
      close(conn);
      _exit(0);
    ...
    if (pid < 0) {
      trace_log("[!] daemon connection fork failed errno=%d\n", errno);
    }
    close(conn);
...
static int umh_main(int argc, char **argv) {
  ...
  trace_reset();
  trace_log("[*] umh identity ready pid=%d uid=%d euid=%d requested_uid=%d\n",
            getpid(), getuid(), geteuid(), allowed_client_uid);
  return daemon_main();
}
